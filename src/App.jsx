...
export default function PahlavonClinic() {
  ...
  const [newDoc, setNewDoc] = useState({ name: "", phone: "", department: DEPARTMENTS[0].name, years: "" });
  ...
  const changePassword = async (ev) => {
    ...
    try {
      ...
      setDoctorInfo((d) => ({ ...d, mustChangePassword: false }));
      setNewPw(""); setNewPw2("");
  ...
  const addDoctor = async (ev) => {
    ...
    try {
      const data = await apiFetch("/api/doctors", {
        method: "POST",
        body: JSON.stringify({ name: newDoc.name, phone: newDoc.phone, department: newDoc.department, years: Number(newDoc.years) || 0 }),
      });
      setLastTempPassword({ name: data.doctor.name, phone: data.doctor.phone, tempPassword: data.tempPassword });
      setNewDoc({ name: "", phone: "", department: DEPARTMENTS[0].name, years: "" });
      reloadDoctorsList();
      ...
      <div style={{ background: C.bg, color: C.text, fontFamily: "Inter, sans-serif" }} className="min-h-screen w-full">
        ...
        <header style={{ background: C.primary }} className="w-full px-6 md:px-12 h-16 flex items-center justify-between">
          <div className="flex items-center gap-2">
            <Medal size={22} color={C.gold} />
            <span style={{ fontFamily: "Fraunces, serif", color: C.bg }} className="text-lg font-bold">Pahlavon — Xodimlar paneli</span>
          </div>
          <button onClick={logout} style={{ color: C.onNavyMuted }} className="text-sm font-medium flex items-center gap-1.5 hover:opacity-80">
            <ArrowLeft size={16} /> {authRole ? "Chiqish" : "Bemor saytiga qaytish"}
          </button>
        </header>
  ...
  return (
    <div style={{ background: C.bg, color: C.text, fontFamily: "Inter, sans-serif" }} className="min-h-screen w-full overflow-x-hidden">
      ...
      <section id="hero" style={{ background: C.primary }} className="w-full relative overflow-hidden">
        <div className="max-w-6xl mx-auto px-6 md:px-12 pt-16 pb-20 md:pt-24 md:pb-28 grid md:grid-cols-[1.2fr_1fr] gap-10 items-center relative z-10">
          <div>
            <Eyebrow>Pahlavon tibbiyot markazi</Eyebrow>
            <h1 style={{ fontFamily: "Fraunces, serif", color: C.bg }} className="text-4xl md:text-6xl font-black leading-[0.98] tracking-tight">
              Sog'liq — <span style={{ color: C.gold, fontStyle: "italic", fontWeight: 700 }}>kurashib</span><br />yutiladigan g'alaba
            </h1>
            <p style={{ color: C.onNavyMuted }} className="max-w-md text-base leading-relaxed mt-6">
            ...
            <div className="flex flex-wrap gap-4 mt-8">
              <button onClick={() => scrollTo("booking")} style={{ background: C.accent, color: "#FFFFFF" }} className="px-6 py-3 rounded-full font-semibold text-sm flex items-center gap-2 hover:brightness-110 transition">
                Navbatga yozilish <ChevronRight size={18} />
              </button>
              <button onClick={() => scrollTo("departments")} style={{ borderColor: "#2E4A6B", color: C.bg }} className="px-6 py-3 rounded-full font-semibold text-sm border hover:bg-white/5 transition">
                Bo'limlar
              </button>
            </div>
            ...
            <div className="flex gap-10 mt-12">
              {[["30+", "yillik tajriba"], ["300 000+", "davolangan bemor"], [String(doctors.length || "—"), "shifokor"]].map(([num, label]) => (
                <div key={label}>
                  ...
                  <div style={{ color: "#7C93AC", fontFamily: "'Space Mono', monospace" }} className="text-[11px] mt-1 uppercase tracking-wide">{label}</div>
                </div>
              ))}
            </div>
          </div>
      ...
      {/* ABOUT */}
      <section id="about" style={{ background: C.bg }} className="w-full px-6 md:px-12 pt-4 pb-20 md:pb-28">
        <div className="max-w-6xl mx-auto grid md:grid-cols-2 gap-12 items-center">
          <div>
            <Eyebrow on="dark">Biz haqimizda</Eyebrow>
            <h2 style={{ fontFamily: "Fraunces, serif", color: C.primary }} className="text-3xl md:text-4xl font-black mb-5">Pahlavon klinikasi haqida</h2>
            ...
            <p style={{ color: C.muted }} className="leading-relaxed mb-6">
          ...
          >
            ...
            <div style={{ background: "linear-gradient(to top, rgba(11,25,41,0.85), transparent 55%)" }} className="absolute inset-0 flex flex-col items-start justify-end p-6">
      ...
      {announcements.length > 0 && (
        <section id="announcements" style={{ background: C.bgAlt }} className="w-full px-6 md:px-12 py-14">
          <div className="max-w-6xl mx-auto">
            <Eyebrow on="dark">E'lonlar</Eyebrow>
            <h2 style={{ fontFamily: "Fraunces, serif", color: C.primary }} className="text-2xl md:text-3xl font-black mb-8">Yangiliklar va e'lonlar</h2>
            <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-5">
              {announcements.map((a) => (
                <div key={a.id} style={{ background: C.surface, borderColor: C.border }} className="rounded-xl border p-5">
                  <h3 style={{ fontFamily: "Fraunces, serif", color: C.text }} className="font-bold mb-1.5">{a.title}</h3>
            ...
            {DEPARTMENTS.map((d, i) => {
              ...
              return (
                <div
                  ...
                  className="flex flex-col sm:flex-row sm:items-center gap-4 sm:gap-8 px-6 py-6 border-b"
                >
                  ...
                  <div className="flex-1">
                    <h3 style={{ fontFamily: "Fraunces, serif", color: C.text }} className="font-bold text-xl">{d.name}</h3>
                    <p style={{ color: C.muted }} className="text-sm leading-relaxed mt-1">{d.desc}</p>
                  </div>
                </div>
              );
            })}
      ...
      <section id="doctors" style={{ background: C.bg }} className="w-full px-6 md:px-12 py-16 md:py-24">
        <div className="max-w-4xl mx-auto">
          <Eyebrow on="dark">Chempionlar reyestri</Eyebrow>
          <h2 style={{ fontFamily: "Fraunces, serif", color: C.primary }} className="text-3xl md:text-4xl font-black mb-2">Shifokorlarimiz</h2>
          <p style={{ color: C.muted }} className="text-sm mb-10">Tajriba yiliga ko'ra saflangan.</p>
          ...
          {doctorsLoading ? (
            ...
            <div className="space-y-3">
              {rankedDoctors.map((doc, i) => (
                <div key={doc.id} style={{ borderColor: C.border, background: C.surface }} className="flex items-center gap-5 rounded-xl border px-5 py-4">
      ...
      {/* BOOKING — teskari kontrast: krem forma navy fonda */}
      <section id="booking" style={{ background: C.primary }} className="w-full px-6 md:px-12 pt-4 pb-20 md:pb-28">
        <div className="max-w-6xl mx-auto grid md:grid-cols-2 gap-12">
          <div>
            <Eyebrow>Navbat</Eyebrow>
            <h2 style={{ fontFamily: "Fraunces, serif", color: C.bg }} className="text-3xl md:text-4xl font-black mb-4">Onlayn navbatga yoziling</h2>
            ...
            <div className="space-y-3">
              {[[Phone, "+998 97 585 28 10"], [Clock, "Dushanba–Shanba, 08:30–17:00 (12:00–13:00 tushlik tanaffusi)"], [MapPin, "Qashqadaryo viloyati, Shahrisabz shahri, 31-G uy"]].map(([Icon, txt]) => (
          ...
          <div style={{ background: C.bg, borderColor: C.border }} className="rounded-2xl border p-7">
            {confirmed ? (
              <div className="flex flex-col items-center text-center py-6">
                <div style={{ background: C.bgAlt }} className="w-14 h-14 rounded-full flex items-center justify-center mb-4"><Check size={26} color={C.primary} /></div>
                <h3 style={{ fontFamily: "Fraunces, serif", color: C.primary }} className="text-xl font-bold mb-1">Navbatingiz qabul qilindi</h3>
                <p style={{ color: C.muted }} className="text-sm mb-4">Navbat raqamingiz: <span style={{ color: C.accent, fontFamily: "'Space Mono', monospace" }} className="font-semibold">{confirmed.number}</span></p>
                <div style={{ background: C.bgAlt }} className="rounded-lg p-4 w-full text-sm text-left space-y-1 mb-5">
                  <p><span style={{ color: C.muted }}>Ism:</span> {confirmed.name}</p>
                  <p><span style={{ color: C.muted }}>Bo'lim:</span> {confirmed.department}</p>
                  <p><span style={{ color: C.muted }}>Sana:</span> {confirmed.date}</p>
                  <p><span style={{ color: C.muted }}>Vaqt:</span> {confirmed.time}</p>
                </div>
                <button onClick={() => setConfirmed(null)} style={{ background: C.primary, color: C.bg }} className="px-5 py-2.5 rounded-full text-sm font-semibold">Yana bir navbat yozish</button>
              </div>
            ) : (
              <form onSubmit={submit} className="space-y-4">
                <div>
                  <label style={{ color: C.text }} className="text-sm font-medium block mb-1.5">To'liq ism</label>
                ...
                <div>
                  <label style={{ color: C.text }} className="text-sm font-medium block mb-1.5">Telefon raqam</label>
                ...
                <div>
                  <label style={{ color: C.text }} className="text-sm font-medium block mb-1.5">Bo'lim</label>
                ...
                <div>
                  <label style={{ color: C.text }} className="text-sm font-medium block mb-1.5">Shifokor (ixtiyoriy)</label>
                ...
                <div className="grid grid-cols-2 gap-3">
                  <div>
                    <label style={{ color: C.text }} className="text-sm font-medium block mb-1.5">Sana</label>
                  ...
                  <div>
                    <label style={{ color: C.text }} className="text-sm font-medium block mb-1.5">Vaqt</label>
                ...
                <div>
                  <label style={{ color: C.text }} className="text-sm font-medium block mb-1.5">Izoh (ixtiyoriy)</label>
                ...
                <button type="submit" disabled={submitting} style={{ background: C.accent, color: "#FFFFFF" }} className="w-full py-3 rounded-full font-semibold text-sm flex items-center justify-center gap-2 hover:brightness-110 transition disabled:opacity-60">
      ...
      <footer id="contact" style={{ background: C.primaryDark }} className="w-full px-6 md:px-12 py-14">
        <div className="max-w-6xl mx-auto grid md:grid-cols-4 gap-10">
          <div>
            <div className="flex items-center gap-2 mb-3"><Medal size={22} color={C.gold} /><span style={{ fontFamily: "Fraunces, serif", color: C.bg }} className="text-lg font-bold">Pahlavon</span></div>
          ...
          <div>
            ...
            <div className="space-y-2">
              {[[Phone, "+998 97 585 28 10"], [MapPin, "Shahrisabz sh., 31-G uy, Qashqadaryo"], [Clock, "Dush–Shan, 08:30–17:00"]].map(([Icon, txt]) => (
        ...
        <div style={{ borderColor: "#1C334E" }} className="max-w-6xl mx-auto border-t mt-10 pt-6">
          <p style={{ color: "#4E698A", fontFamily: "'Space Mono', monospace" }} className="text-xs text-center">© {new Date().getFullYear()} Pahlavon tibbiyot markazi. Barcha huquqlar himoyalangan.</p>
        </div>
      </footer>
    </div>
  );
